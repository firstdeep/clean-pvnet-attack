...
def depth_im_to_dist_im(depth_im, K):
    ...
    Xs = np.multiply(xs - K[0, 2], depth_im) * (1.0 / K[0, 0]) # 1 ~ 720 - camera center pose * depth img * 1/Focal length_X : Focal length == X/Z
    # Focal length 로 나눔으로써 normal image plane 으로 가져오고 depth 이미지를 곱한다. 그러므로 카메라 중심점으로 부터의 distance image가 완료된다.
