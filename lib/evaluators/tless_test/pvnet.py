...
class Evaluator:
    ...
    def vsd_metric(self, pose_pred, pose_gt, K, depth_path, icp=False):
        ...
        delta = tless_config.vsd_delta # 15
        tau = tless_config.vsd_tau # 20
        cost_type = tless_config.vsd_cost # step
        error_thresh = tless_config.error_thresh_vsd # 0.3
        ...
        for pose_pred_ in pose_pred:
            ...
            for gt_id, pose_gt_ in enumerate(pose_gt):
                ...
                if gt_id not in visib_gt:
                    ...
                    # val_depth_max = np.max(depth_gt_)
                    # a = (depth_gt_ / val_depth_max) * 255
                    # cv2.imshow("depth", a)
                    # cv2.waitKey(0)
