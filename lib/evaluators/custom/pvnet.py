...
class Evaluator:
    ...
    def __init__(self, result_dir):
        ...
        obj_path = os.path.join(model_dir, 'obj_{:02d}.ply'.format(cfg.tless_cls))
        self.model_ = inout.load_ply(obj_path)
        self.model = self.model_['pts'] / 1000.

        model_info = yaml.load(open(os.path.join(model_dir, 'models_info.yml')))
        self.diameter = model_info[cfg.tless_cls]['diameter'] / 1000.
        #############################################################################
